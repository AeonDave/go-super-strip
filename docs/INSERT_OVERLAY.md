...
The `-i` (section insertion) and `-l` (overlay/loader) stages let users embed payloads inside a binary. They are often combined with obfuscation and packing. This doc explains the grammar, encryption support, and PE/ELF specifics.
...
- Provide exactly one of `data` (inline ASCII string, or raw hex bytes prefixed with `0x`, e.g., `data=0x414243`) or `file` (path on disk).
- Optional `password` encrypts the payload using AES-256-GCM with a PBKDF2-derived key and a random IV.
...
- Developers can run `test/cli_matrix.sh` (Linux/WSL) to capture analyze output and ensure inserted sections are recognized.
